...
class MLP():
    def __init__(self):
        self.hidden_layer = [Perceptron(2), Perceptron(
            2), Perceptron(2), Perceptron(2)]
    ...
    def backpropagate(self, target):
        ...
        # save the deltas in the right order
        deltas = []
    ...
    def forwardpropagate(self, input):
        ...
        previous_activations_reshaped = np.reshape(
            self.previous_activations, newshape=(-1))
        self.output = self.output_layer.forward_step(
            previous_activations_reshaped)
